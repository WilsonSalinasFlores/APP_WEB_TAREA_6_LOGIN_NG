...
export class ClienteService {
  private readonly rutaAPI = 'https://localhost:7004/api/Cliente';
  ...
  editarCliente(cliente:ICliente): Observable<ICliente> {
    return this.http.put<ICliente>(`${this.rutaAPI}/${cliente.id}`, cliente).pipe(
      catchError(this.manejoErrores)
    ); 
  }
  ...
  uncliente(id:number): Observable<ICliente> {
    return this.http.get<ICliente>(`${this.rutaAPI}/${id}`).pipe(
      catchError(this.manejoErrores)
    ); 
  }
}